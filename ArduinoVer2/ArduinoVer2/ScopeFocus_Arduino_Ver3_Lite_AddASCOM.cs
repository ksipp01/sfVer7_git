// Arduino Lite version 11-25-11
// Kevin Sipprell MD
// www.scopefocus.info
// info@scopefocus.info
...
/*
       public void Send(string msg)//needed becuase other version have error messaging. 
       {
           return;
       }
...
//standby
       ...
       public void gotopos(Int32 value)
        {
           /*
            if (focuser.Absolute == false)//relative mode
            {
                
                int relpos = count + value;
                focuser.Move(relpos);
                count = count + value;
      ...
      //     {
            ...
            count = focuser.Position;
                focuser.Move(value);
                int steps = Math.Abs(count - value);
              //  while (focuser.IsMoving) //  ???????????problem w/ sgl and ismoving
             //   {
                   // Thread.Sleep(2 * steps);
               //     Thread.Sleep(1000);
                 //   Log("focuser.IsMoving = " + focuser.IsMoving.ToString());
            //    }
    ...
    //Begin slope calc based on entered min max array*****************
                        ...
                        //end course v-curve and goes to rough focus point
                         ...
                         if ((tempon == 1) & (vDone == 1))
                         {
                             ...
                             int finegoto = posMin - ((((int)numericUpDown3.Value) / 2) * ((int)numericUpDown8.Value));
                           //  if ((finegoto - 100) < 0)
                             if ((finegoto - ((int)numericUpDown8.Value * 3)) < 0)
                             {
...
public void gotoFocus()
{
    ...
    {//this was original 
        ...
        gotopos(Convert.ToInt32(posMin + (int)numericUpDown11.Value * 2));
        Thread.Sleep(3000);
        gotopos(Convert.ToInt32(posMin + (int)numericUpDown11.Value));
        Thread.Sleep(3000);
    }
...
//    if (portopen == 1)
  //  {     
   ...
   if ((finegoto - ((int)numericUpDown8.Value * 3)) < 0)
        ...
        Log("Taking Up Backlash...");
    /*
        gotopos(Convert.ToInt32(finegoto - ((int)numericUpDown8.Value) * 5));
...
private void button8_Click_2(object sender, EventArgs e)
{
   ...
   if (connect == 1)
   {
 //removed a ton of arduino sync data code from here
...
private void button14_Click(object sender, EventArgs e)
{
    DialogResult result = MessageBox.Show("Reset to Defaults? ", "scopefocus",
     ...
     else
     {
         ...
         WindowsFormsApplication1.Properties.Settings.Default.setup1 = "Enter setup info";
         WindowsFormsApplication1.Properties.Settings.Default.setup2 = "Enter setup info";
         WindowsFormsApplication1.Properties.Settings.Default.setup3 = "Enter setup info";
         WindowsFormsApplication1.Properties.Settings.Default.setup4 = "Enter setup info";
         WindowsFormsApplication1.Properties.Settings.Default.setup5 = "Enter setup info";
         WindowsFormsApplication1.Properties.Settings.Default.setup6 = "Enter setup info";
...
private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
{
    ...
    if ((listView1.CheckedItems.Count != 0) && (tabControl1.SelectedTab == tabPage1))
    {
        ...
        textBox2.Text = focuser.MaxStep.ToString();
...
{
    posMin = Convert.ToInt32(textBox4.Text);
    roughvdone = true;
}
