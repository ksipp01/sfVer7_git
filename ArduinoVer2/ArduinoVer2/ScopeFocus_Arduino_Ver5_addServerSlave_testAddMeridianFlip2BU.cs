...
//************** 3-8-13 for meridian flip...cant monitor anyting w/ this while statement is going ******
 //************  may need to change so sytemfilewatch4 monitors subs and counts rathing than waiting for neb status change ****                   
              ...
              //     {
                        ...
                        //2 lines below should be deleted if 3-11-13 rem is removed
                        if (backgroundWorker2.IsBusy != true)
                            backgroundWorker2.RunWorkerAsync();

                     // remd 3-11-13 moved to fswatcher4  CheckForFlip();
                      /*  
...
//****try add the whole thing****
           ...
           //******check for meridian flip here ???????******  3-8-13
                        ...
                        */
                        /*//3-11-13 changed to monitornebstatus in background2
                        int StatusstripHandle = FindWindowEx(NebhWnd, 0, "msctls_statusbar32", null);
...
*/
       ...
       */
        ...
        private void button19_Click_2(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK)
            {
                Capturing = false;//added 3-11-13
                backgroundWorker2.CancelAsync();//added 3-11-13
                fileSystemWatcher4.EnableRaisingEvents = false;
                filterCountCurrent = 0;
                subCountCurrent = 0;
      ...
      //  TimeSpan TimeToFlip;
        ...
        {         
            ...
            if (checkBox24.Checked == true)//correct for DST
                zone = zone + 1;
            correction = (Longitude / 15 - zone) * 60;

            DateTime saveNow = DateTime.Now;
            GST =  time.CalcGSTFromUT(saveNow);
            TimeDec = time.ConvTimeToDec(GST);
            Urania = time.ConvDecTUraniaTime(TimeDec);
            ...
            DateTime Uhms = Convert.ToDateTime(currentU);
            textBox44.Text = currentU.ToString();
            textBox45.Text = saveNow.ToString("HH:mm:ss");
            textBox46.Text = time.GetJulianDay(saveNow, zone).ToString();//zone is -6
            textBox47.Text = time.CalcUTFromZT(saveNow, zone).ToString("HH:mm:ss");
            while (MountMoving == true)
            {
                Thread.Sleep(1000);
                Log("waiting for slew to complete");
            }
            //get current locaton
            if (port2 == null)
            {
            ...
            if (CurrentLoc.Substring(17, 1) == "#")//check for stop bit
            {
                ...
                textBox50.Text = FlipTime.ToString("HH:mm:ss");
                textBox48.Text = TimeCompare.ToString();
                if (FlipTextSent == false & TimeToFlip.Hours < 1 & TimeToFlip.Minutes * 60 < CaptureTime3)
                {
                    Send("Meridian flip after next sub in " + (CaptureTime3/60).ToString() + " minutes");
                    FlipTextSent = true;
                }
                if (!File.Exists(fullpath))
                {
        ...
        private void checkBox23_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                CheckForFlip();
                if (!TargetLocObtained)
                {
                    DialogResult result;
                    result = MessageBox.Show("Target location not set - set to current location?",
                       "scopefocus", MessageBoxButtons.OKCancel, MessageBoxIcon.Exclamation);//change so ok moves focus
                    if (result == DialogResult.OK)
                        GetTargetLocation();
                    if (result == DialogResult.Cancel)
                    {
                ...
                {
                    okToFlip = true;
                    DialogResult result1 = MessageBox.Show("Merdian Flip at " + FlipTime.ToString("HH:mm:ss"), "scopefocus", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    if (result1 == DialogResult.OK)
                    {
                        return;
                    }
        ...
        public static int PHD_FLIP = 16;
        private void PHDflip()    // *****untested****** 3-12-13
        {
            try
            {
                PHDSocketPause(true);
                Thread.Sleep(500);
                byte[] buf = new byte[1];
                buf[0] = (byte)((char)PHD_FLIP);
                phdsocket.Client.Send(buf);
                phdsocket.Client.Receive(buf);
                Thread.Sleep(500);
                PHDSocketPause(false);
                Log("PHD calibration data - flipped");
        ...
        private void button49_Click(object sender, EventArgs e)
        {
            ...
            ASCOM.DriverAccess.Telescope.Choose(id);
        ...
        {
            Telescope scope = new Telescope(driverID);
            scope.SlewToAltAz(RA, Dec);
        }
        ...
        private void button50_Click(object sender, EventArgs e)
        {
            Telescope scope = new Telescope(id);
