...
//******textBox 3 got changed from temp to Entered slopeDWN ***** need to fix eventually****
using System;
           ...
           private void fileSystemWatcher3_Changed(object sender, FileSystemEventArgs e)
           {
               ...
               if (portopen == 1)
               {
                    ...
                    if (vProgress == 10)
                   {
                       if (GotoFocusOn == true)
                       {
                           double EnteredSlopeUP;//need to add downslope and method to identify which side of focus starting from 
                           int EnteredPID = Convert.ToInt16(textBox12.Text);
                           EnteredSlopeUP = Convert.ToDouble(textBox10.Text);
                           textBox14.Text = avg.ToString();
                           double BestPos = count - (avg / EnteredSlopeUP) + (EnteredPID / 2);
                           gotopos(Convert.ToInt16(BestPos));//not converting to correct int  and ?  not stopping 
                           fileSystemWatcher3.EnableRaisingEvents = false;
                           GotoFocusOn = false;
...
//try multiple point slope determination then uses 2 points up from apex on both sides to calc posMin                        
                                 ...
                                 //test GetSlope
                                 //hfr array     
                                 if ((apexHFR > (vN/2 +4)) || (apexHFR < (vN/2 -4)))
                                 {
                                     DialogResult result2 = MessageBox.Show("V-Curve Not Aymmetric", "Aborted - Repeat Rough V-Curve", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                     if (result2 == DialogResult.OK)
                                     {
                                         standby();
                                     }
                                     ...
                                     int downN = apexHFR -1;
                                 double[] HFRdown = new double[vN/2 - 4];
                                 double[] HFRup = new double[vN/2 - 4];
                                 double[] HFRposdwn = new double[vN/2 - 4];
                                 double[] HFRposup = new double[vN/2 - 4];//was vN-apexHFR-3
                                 for (int dwn = 0; dwn < (vN/2-4); dwn++)
                                 {
                                     HFRdown[dwn] = list[((apexHFR-(vN/2-5))+dwn)];
                                     HFRposdwn[dwn] = minHFRpos[((vN/2 -5)+dwn)];
                                 }
                                 for (int up = 0 ; up < vN/2-4; up++)
                                 {
                                     HFRup[up] = list[apexHFR + 2 + up];
                                     HFRposup[up] = minHFRpos[apexHFR +2 + up];
                                 }
                                 slopeHFRdwn = (1/GetSlope(HFRdown, HFRposdwn));
...
*/
                                 ...
                                 posMin = posminHFR;
                                 intersectPos = (int)GetIntersectPos((double)minHFRpos[apexHFR - 5], (double)minHFRpos[apexHFR + 5], (double)list[apexHFR - 5], (double)list[apexHFR + 5], slopeHFRdwn, slopeHFRup);
                          //       posMin = (int)(count - (current / slopeHFRup) + (PID / 2));
                                     textBox4.Text = posMin.ToString();
                ...
                //         textBox12.Text = maxarrayMax.ToString();
                       //  textBox13.Text = posmaxMax.ToString();
                         ...
                         if ((radioButton1.Checked) & (ffenable ==1))
                         {
                             ...
                             //log data saves text file data.txt to import into excel, tab delimited
...
//added to sync for manual changes
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (portopen == 1)
            {
                ...
                chart1.Series[1].Points.Clear();
                chart1.Series[2].Points.Clear();
                gotopos(Convert.ToInt16(posMin + 200));//take up backlash
                Thread.Sleep(3000);
                int finegoto = posMin - ((((int)numericUpDown3.Value) / 2) * ((int)numericUpDown8.Value));
                //fine v-curve goes to N/2 * step size in from the focus position -- V should be centered
                gotopos(Convert.ToInt16(finegoto));
